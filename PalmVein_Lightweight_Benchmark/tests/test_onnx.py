...
class OnnxTests(unittest.TestCase):
    def test_export_checker_and_runtime_parity(self):
        model = build_ding_pruned(4).eval()
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            report = compare_outputs(expected, path, sample.numpy().astype(np.float32), atol=1e-4, rtol=1e-3)
    ...
    def test_static_qdq_int8_smoke(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            torch.onnx.export(model, torch.from_numpy(sample), str(fp32), input_names=["input"], output_names=["logits"], opset_version=13)
            quantize_static(
                str(fp32), str(int8), Reader([sample, sample * 0.5]), quant_format=QuantFormat.QDQ,
